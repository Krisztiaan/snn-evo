...
def test_basic_functionality():
    ...
    assert len(state.reward_positions) == 5, f"Should have 5 rewards, got {len(state.reward_positions)}"
    ...
    assert new_pos == (initial_pos[0] + 1, initial_pos[1]), f"Agent should move right, got {new_pos}"
...
def test_reward_collection():
    ...
    assert result.reward == config.reward_value, f"Should get reward value {config.reward_value}, got {result.reward}"
    assert result.state.total_reward == config.reward_value, "Total reward should update"
    assert ~result.state.reward_collected[0], "Collected reward should respawn (collected flag reset)"
    
    # Verify reward respawned at different position
    new_reward_pos = tuple(result.state.reward_positions[0])
    assert new_reward_pos != reward_pos, f"Reward should respawn at different position, still at {new_reward_pos}"
    ...
    assert result.reward == config.proximity_reward, f"Should get proximity reward {config.proximity_reward}, got {result.reward}"
...
def test_observation_gradient():
    ...
    assert gradients[0] >= gradients[1] >= gradients[2], f"Gradients should decrease with distance: {gradients}"
