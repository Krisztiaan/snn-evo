...
class SimpleGridWorld:
    ...
    def _reset_jit(self, key: random.PRNGKey) -> Tuple[WorldState, Observation]:
        ...
        observation = self._get_observation_fast(agent_packed, reward_positions_packed, state.reward_collected)
    ...
    def _step_jit(self, state: WorldState, action: int, key: random.PRNGKey) -> StepResult:
        ...
        respawn_offset = jnp.sum(at_reward.astype(jnp.int32)) * 37  # Prime for better distribution
        ...
        # Get observation
        observation = self._get_observation_fast(new_agent_packed, new_reward_packed, final_collected)
