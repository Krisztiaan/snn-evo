...
def main():
    ...
    for config_name, config in configs:
        ...
        print(f"    Steps: {results['v1']['avg_step_time'] / results['v2']['avg_step_time']:.1f}x faster")
        
        print(f"  V3 vs V1:")
        print(f"    Reset: {results['v1']['reset_time'] / results['v3']['reset_time']:.1f}x faster")
        print(f"    Steps: {results['v1']['avg_step_time'] / results['v3']['avg_step_time']:.1f}x faster")
        
        print(f"  V3 vs V2:")
        print(f"    Reset: {results['v2']['reset_time'] / results['v3']['reset_time']:.1f}x faster")
        print(f"    Steps: {results['v2']['avg_step_time'] / results['v3']['avg_step_time']:.1f}x faster")
        
        # Verify correctness (same rewards collected)
        print(f"\n✅ Correctness check:")
        print(f"  Total rewards collected: V1={results['v1']['total_reward']:.0f}, "
    ...
    for version_name, world_class in [("V1", WorldV1), ("V2", WorldV2), ("V3", WorldV3)]:
        ...
        for size in grid_sizes:
            ...
            try:
                ...
                print(f"  {size}x{size}: {format_time(results['avg_step_time'])} per step "
