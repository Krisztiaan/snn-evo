...
class SimpleGridWorld:
    ...
    def _generate_rewards(self, key: random.PRNGKey, agent_pos: Tuple[int, int]) -> Array:
        ...
        if self.config.n_rewards < 10:
            ...
            positions = random.randint(
                key1, shape=(n_candidates, 2), minval=0, maxval=self.config.grid_size
            )
        else:
            ...
            cluster_centers = random.uniform(
                key1, shape=(n_clusters, 2), minval=0, maxval=self.config.grid_size
            ).astype(jnp.int32)
    ...
    def _collect_rewards(
        ...
        new_reward_positions, new_collected = jax.lax.cond(
            any_collected, respawn_rewards, lambda p, c: (p, c), reward_positions, new_collected
        )
    ...
    def _get_observation(self, state: WorldState) -> Observation:
        ...
        gradient = jnp.where(
            jnp.isfinite(min_distance), jnp.exp(-min_distance / decay_constant), 0.0
        )
    ...
    def _calculate_distances(self, pos: Array, positions: Array) -> Array:
        ...
        if self.config.toroidal:
            ...
            diff = positions - pos
            wrapped_diff = jnp.minimum(jnp.abs(diff), self.config.grid_size - jnp.abs(diff))
    ...
    def _find_farthest_position(
        self, agent_pos: Tuple[int, int], reward_positions: Array, reward_collected: Array
    ) -> Array:
        ...
        # Shape: (n_candidates, 1, 2)
        candidate_positions_expanded = candidate_positions[:, None, :]
        # Shape: (1, n_rewards, 2)
        reward_positions_expanded = reward_positions[None, :, :]
        ...
        masked_distances = jnp.where(reward_collected[None, :], jnp.inf, pairwise_distances)
        ...
        scores = distances_from_agent + 0.2 * jnp.where(
            jnp.isfinite(min_distances_to_rewards), min_distances_to_rewards, 0.0
        )
