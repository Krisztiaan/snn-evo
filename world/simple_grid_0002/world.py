...
class SimpleGridWorld:
    ...
    def _generate_rewards_static(self, key: random.PRNGKey, agent_pos: Tuple[int, int]) -> Array:
        ...
        grid_ranks = jnp.where(grid_valid_mask, jnp.arange(len(grid_positions)), len(grid_positions) + jnp.arange(len(grid_positions)))
        ...
        def select_position(i):
            ...
            return jax.lax.cond(
                use_selected,
                lambda: selected_positions[i],
                lambda: ranked_grid[grid_idx]
            )
    ...
    def _collect_rewards_static(
        ...
        def update_single_reward(carry, i):
            ...
            new_positions = jax.lax.cond(
                should_update,
                lambda: positions.at[i].set(new_spawn_pos),
                lambda: positions
            )
            return new_positions, None
        
        new_reward_positions, _ = jax.lax.scan(
            update_single_reward,
            reward_positions,
            jnp.arange(self.config.n_rewards)
        )
        
        # Reset collected status for respawned rewards
        final_collected = jax.lax.cond(
            any_collected,
            lambda: new_collected & ~at_reward,
            lambda: new_collected
        )
    ...
    def _get_observation(self, state: WorldState) -> Observation:
        ...
        gradient = jnp.where(
            ...
            jnp.exp(-min_distance / self.decay_constant),
            0.0
        )
