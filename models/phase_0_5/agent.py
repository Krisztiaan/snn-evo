...
def _learning_step(state: AgentState, reward: float, params: NetworkParams) -> AgentState:
    ...
    # 3. Weight Update
    dopamine_modulation = jax.nn.tanh((new_dopamine - params.BASELINE_DOPAMINE) / (params.BASELINE_DOPAMINE + 1e-8) * 2.0)
...
class SnnAgent:
    ...
    def _initialize_state(self, key: random.PRNGKey) -> AgentState:
        # ... (complex initialization logic ported from phase04_enhanced.py)
    ...
    def run_episode(self, episode_key: random.PRNGKey, episode_num: int) -> Dict[str, Any]:
        """Run a single episode, returning a summary and handling data export."""
        world_state, obs = self.world.reset(episode_key)
        self.state = self._initialize_state(episode_key) # Re-initialize agent state for each episode
        
        exporter = None
        if self.config.exp_config.enable_export:
            exporter = DataExporter(
                experiment_name=f"phase05_e{episode_num:03d}",
                output_base_dir=self.config.exp_config.export_dir,
                compression='gzip', compression_level=1
            )
            exporter.save_config(self.config._asdict())
        ...
        for step in range(self.config.world_config.max_timesteps):
            ...
            # World interaction
            action = int(random.categorical(decision_key, jnp.ones(self.params.NUM_OUTPUTS))) # Simplified action for now
            ...
            if exporter:
                exporter.log(
                    ...
                    behavior={"action": action, "pos_x": world_state.agent_pos[0], "pos_y": world_state.agent_pos[1]},
                    reward=reward
                )
