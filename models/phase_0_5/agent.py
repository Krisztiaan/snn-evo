...
def _neuron_step(state: AgentState, params: NetworkParams, key: random.PRNGKey) -> AgentState:
    ...
    i_total = i_ext + syn_current_e_new + syn_current_i_new + \
        random.normal(key, state.v.shape) * 1.0
    ...
    return state._replace(
        ...
        trace_fast=trace_fast_new, firing_rate=firing_rate_new,
...
def _learning_step(state: AgentState, reward: float, params: NetworkParams) -> AgentState:
    ...
    dopamine_response = state.dopamine * decay + \
        params.BASELINE_DOPAMINE * (1 - decay) + td_error * 0.5
    ...
    stdp = jnp.where(state.connection_mask, params.STDP_A_PLUS *
                     ltp - params.STDP_A_MINUS * ltd, 0.0)
    ...
    dw = effective_lr * dopamine_modulation * \
        new_eligibility - params.WEIGHT_DECAY * state.w
...
class SnnAgent:
    ...
    def _initialize_state(self, key: random.PRNGKey) -> AgentState:
        ...
        dist_matrix = jnp.minimum(jnp.abs(
            positions[:, None] - positions[None, :]), n - jnp.abs(positions[:, None] - positions[None, :]))
        ...
        prob_matrix = jnp.where(
            jnp.outer(is_excitatory, ~is_excitatory), p.P_EI, prob_matrix)
        prob_matrix = jnp.where(
            jnp.outer(~is_excitatory, is_excitatory), p.P_IE, prob_matrix)
        prob_matrix = jnp.where(
        ...
        # Weights
        w = jnp.zeros((n, n))
        w = jnp.where(jnp.outer(is_excitatory, is_excitatory) & connection_mask, random.normal(
            keys[1], (n, n)) * p.W_EE_MEAN * p.W_STD_SCALE + p.W_EE_MEAN, w)
        w = jnp.where(jnp.outer(is_excitatory, ~is_excitatory) & connection_mask, random.normal(
            keys[2], (n, n)) * p.W_EI_MEAN * p.W_STD_SCALE + p.W_EI_MEAN, w)
        w = jnp.where(jnp.outer(~is_excitatory, is_excitatory) & connection_mask, random.normal(
            keys[3], (n, n)) * p.W_IE_MEAN * p.W_STD_SCALE + p.W_IE_MEAN, w)
        w = jnp.where(jnp.outer(~is_excitatory, ~is_excitatory) & connection_mask, random.normal(
            keys[4], (n, n)) * p.W_II_MEAN * p.W_STD_SCALE + p.W_II_MEAN, w)
        w = _apply_dale_principle(w, is_excitatory)

        # Input weights
        input_centers = jnp.linspace(0, 1, p.NUM_INPUTS)
        neuron_prefs = jnp.linspace(0, 1, n)
        w_in = jnp.exp(-((input_centers[None, :] -
                       neuron_prefs[:, None])**2) / (2 * 0.15**2))
        w_in = (w_in + random.normal(keys[5], w_in.shape) *
                0.1) / (jnp.sum(w_in, axis=1, keepdims=True) + 1e-8)
        ...
        return AgentState(
            ...
            is_excitatory=is_excitatory, firing_rate=jnp.full(
                n, p.TARGET_RATE_HZ),
            threshold_adaptation=jnp.zeros(n), syn_current_e=jnp.zeros(n), syn_current_i=jnp.zeros(n),
            trace_fast=jnp.zeros(n), eligibility_trace=jnp.zeros((n, n)), dopamine=p.BASELINE_DOPAMINE,
            learning_rate_factor=jnp.ones((n, n)), w=w, w_in=w_in.T, w_out=w_out,
            connection_mask=connection_mask, value_estimate=0.0, prev_value=0.0,
    ...
    def run_episode(self, episode_key: random.PRNGKey, episode_num: int, exporter: DataExporter) -> Dict[str, Any]:
        ...
        for step in range(self.config.world_config.max_timesteps):
            ...
            # World interaction
            action = int(random.categorical(decision_key, jnp.ones(
                self.params.NUM_OUTPUTS)))  # Simplified action for now
            ...
            exporter.log(
                timestep=step,
                neural_state={"v": self.state.v,
                              "spikes": self.state.spike.astype(jnp.uint8)},
                behavior={"action": action, "pos_x": int(
                    world_state.agent_pos[0]), "pos_y": int(world_state.agent_pos[1])},
                reward=float(reward)
            )
    ...
    def run_experiment(self):
        ...
        with DataExporter(
            ...
            exporter.save_network_structure(
                neurons={'neuron_ids': np.arange(
                    self.params.NUM_NEURONS), 'is_excitatory': np.array(self.state.is_excitatory)},
                connections={'source_ids': np.where(self.state.connection_mask)[
                    0], 'target_ids': np.where(self.state.connection_mask)[1]},
                initial_weights={'weights': np.array(
                    self.state.w[self.state.connection_mask])}
            )
            ...
            for i in range(self.config.exp_config.n_episodes):
                ...
                summary = self.run_episode(
                    episode_key, episode_num=i, exporter=exporter)
