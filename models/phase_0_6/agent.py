...
def _update_metaplasticity(state: AgentState, params: NetworkParams) -> AgentState:
    ...
    return state._replace(synapse_age=new_age, learning_rate_factor=age_factor)
...
class SnnAgent:
    ...
    def _initialize_state(self, key: random.PRNGKey) -> AgentState:
        ...
        w_out = jnp.where(
            is_excitatory[:, None], jnp.abs(random.normal(keys[6], (n, p.NUM_OUTPUTS)) * 0.1), 0.0
        )
