# models/phase_0_11/agent.py
# keywords: [snn agent, phase 0.11, optimized, precomputed exponentials, efficient]
"""
Phase 0.11 SNN Agent: Optimized version with performance improvements

Key optimizations:
1. Precomputed exponential decay factors
2. Reduced array allocations in hot paths
3. Efficient spike handling without type conversions
4. Optimized matrix operations
5. Proper PRNG key management
"""
...
def _neuron_step_optimized(
    ...
    firing_rate_new = state.firing_rate * (1 - constants.homeostatic_alpha) + spike_rate * constants.homeostatic_alpha
    ...
    threshold_adapt_new = jnp.clip(threshold_adapt_new, -params.MAX_THRESHOLD_ADAPT, params.MAX_THRESHOLD_ADAPT)
...
def _three_factor_learning_optimized(
    ...
    """
    Three-factor learning rule with optimizations.
    """
    ...
    dopamine_response = state.dopamine * constants.dopamine_decay + params.BASELINE_DOPAMINE * (1 - constants.dopamine_decay) + total_reward_signal
    ...
    for i, sens_idx in enumerate(sens_indices):
        for j, proc_idx in enumerate(proc_indices):
            if w_mask[proc_idx, n_in + sens_idx]:
                w_val = jnp.abs(random.normal(keys[1], ()) * params.W_SENSORY_PROC * params.W_INIT_SCALE + params.W_SENSORY_PROC)
    ...
    for i, src_idx in enumerate(proc_indices):
        if is_excitatory[src_idx]:  # Only E neurons
            for j, tgt_idx in enumerate(readout_indices):
                if w_mask[tgt_idx, n_in + src_idx]:
                    w_val = jnp.abs(random.normal(keys[3], ()) * params.W_PROC_READOUT * params.W_INIT_SCALE + params.W_PROC_READOUT)
...
def _consolidate_weights(state: AgentState, params: NetworkParams) -> AgentState:
    ...
    scale_factor = jnp.where(
        w_mean > 0,
        1.0 + (target_mean / w_mean - 1.0) * params.WEIGHT_CONSOLIDATION_RATE,
        1.0
    )
...
class SnnAgent:
    """Phase 0.11 SNN Agent with performance optimizations."""
    ...
    def _initialize_state(self, key: random.PRNGKey) -> AgentState:
        ...
        w_mask, w_plastic_mask = create_connectivity_vectorized(keys[0], p, n_total, is_excitatory, neuron_types)
    ...
    def run_episode(self, episode_key: random.PRNGKey, episode_num: int, exporter: Optional[DataExporter] = None, 
                    progress_callback=None, performance_mode: bool = False) -> Dict[str, Any]:
        ...
        for step in range(max_steps):
            ...
            self.state, spike_float = _neuron_step_optimized(self.state, self.params, self.constants, neuron_key)
            
            # 3. Decode action (optimized)
            action, motor_trace = _decode_action_optimized(self.state, self.params, self.constants, action_key)
    ...
    def run_experiment(self, performance_mode: bool = False, no_write: bool = False):
        ...
        if not performance_mode:
            ...
            exporter = DataExporter(
                experiment_name="snn_agent_phase11_optimized",
        ...
        for i in range(self.config.exp_config.n_episodes):
            ...
            summary = self.run_episode(episode_key, episode_num=i, exporter=exporter, 
                                     progress_callback=progress_callback, performance_mode=performance_mode)
