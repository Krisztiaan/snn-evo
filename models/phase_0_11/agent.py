...
def _encode_gradient_population(gradient: float, params: NetworkParams, key: random.PRNGKey) -> jnp.ndarray:
    ...
    normalized = jnp.where(
        ...
        jnp.ones_like(activations) * params.INPUT_GAIN /
...
def _compute_eligibility_trace_optimized(
    ...
    pre_trace = constants.zero_input_buffer.at[params.NUM_INPUT_CHANNELS:].set(
        state.trace_fast)
...
def _three_factor_learning_optimized(
    ...
    dopamine_response = state.dopamine * constants.dopamine_decay + \
        params.BASELINE_DOPAMINE * \
        (1 - constants.dopamine_decay) + total_reward_signal
    ...
    new_eligibility = _compute_eligibility_trace_optimized(
        state, params, constants)
...
class SnnAgent:
    ...
    def _precompute_constants(self) -> PrecomputedConstants:
        ...
        for i in range(4):
            ...
            motor_decode_matrix = motor_decode_matrix.at[i, start_idx:end_idx].set(
                1.0)
    ...
    def run_episode(self, episode_key: random.PRNGKey, episode_num: int, exporter: Optional[DataExporter] = None,
        ...
        for step in range(max_steps):
            ...
            encode_key, neuron_key, action_key, world_key = random.split(
                step_key, 4)
