...
def create_experiment_configs():
    ...
    default_config = SnnAgentConfig(
        ...
        exp_config=ExperimentConfig(
            n_episodes=10,
            seed=42,
            export_dir="experiments/phase_0_11"
        )
    )
...
def benchmark_performance(config):
    ...
    output_dir = Path(config.exp_config.export_dir).parent / \
        "benchmark_results"
...
def main():
    parser = argparse.ArgumentParser(
        description='Run Phase 0.11 SNN experiments')
    parser.add_argument('--config', choices=['default', 'benchmark', 'learning'],
                        default='default', help='Configuration to run')
    ...
    parser.add_argument('--no-write', action='store_true',
                        help='Skip all file I/O operations (for benchmarking)')
    ...
    if args.episodes is not None:
        config = config._replace(
            exp_config=config.exp_config._replace(n_episodes=args.episodes)
        )

    if args.seed is not None:
        config = config._replace(
            exp_config=config.exp_config._replace(seed=args.seed)
        )

    if args.export_dir is not None:
        config = config._replace(
            exp_config=config.exp_config._replace(export_dir=args.export_dir)
        )
    ...
    print(f"Network size: {config.network_params.NUM_SENSORY + config.network_params.NUM_PROCESSING + config.network_params.NUM_READOUT} neurons")
    print(
        f"World size: {config.world_config.grid_size}x{config.world_config.grid_size}")
    ...
    if len(summaries) > 0:
        ...
        avg_collected = sum(s['rewards_collected']
        ...
        if len(summaries) >= 5:
            ...
            print(
                f"Improvement: {improvement:+.2f} ({improvement/early_avg*100:+.1f}%)")
