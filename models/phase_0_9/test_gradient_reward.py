...
def test_gradient_dopamine():
    ...
    for grad in gradients:
        ...
        new_state = _three_factor_learning(state, reward=0.0, gradient=float(grad), params=params)
    ...
    for reward in [0.0, 0.5, 1.0]:
        ...
        for grad in gradients:
            new_state = _three_factor_learning(state, reward=reward, gradient=float(grad), params=params)
    ...
    plt.axhline(y=params.BASELINE_DOPAMINE, color='k', linestyle='--', alpha=0.5)
    plt.xlabel('Gradient Value', fontsize=12)
    plt.ylabel('Dopamine Level', fontsize=12)
    ...
    print(f"Dopamine range tested: {min(dopamine_levels):.3f} - {max(dopamine_levels):.3f}")
