...
def initialize_enhanced_network(key: jax.random.PRNGKey, params: NetworkParams) -> EnhancedNetworkState:
    ...
    return EnhancedNetworkState(
        ...
        firing_rate=firing_rate, threshold_adaptation=threshold_adaptation,
        syn_current_e=syn_current_e, syn_current_i=syn_current_i,
        trace_fast=trace_fast, trace_slow=trace_slow,
        eligibility_trace=eligibility_trace, dopamine=dopamine,
        synapse_age=synapse_age, learning_rate_factor=learning_rate_factor,
...
@jit
def create_structured_connectivity(key: jax.random.PRNGKey, params: NetworkParams, 
                                   is_excitatory: jnp.ndarray) -> jnp.ndarray:
...
@jit
def initialize_synaptic_weights(key: jax.random.PRNGKey, params: NetworkParams,
                                 is_excitatory: jnp.ndarray, 
                                 connection_mask: jnp.ndarray) -> jnp.ndarray:
    """Initialize weights with connection-type specific distributions"""
    n = params.NUM_NEURONS
...
@jit 
def encode_gradient_population(gradient: float, params: NetworkParams, key: jax.random.PRNGKey) -> jnp.ndarray:
...
@jit
def neuron_step_enhanced(state: EnhancedNetworkState, params: NetworkParams, 
                         key: jax.random.PRNGKey) -> EnhancedNetworkState:
...
@jit
def compute_eligibility_trace_enhanced(state: EnhancedNetworkState, 
                                       params: NetworkParams) -> jnp.ndarray:
...
@jit
def update_metaplasticity(state: EnhancedNetworkState, params: NetworkParams) -> EnhancedNetworkState:
    ...
    return state._replace(
        synapse_age=new_age,
        learning_rate_factor=new_learning_rate_factor
    )


@jit
def three_factor_update_enhanced(state: EnhancedNetworkState, 
                                 dopamine_modulation: float,
                                 params: NetworkParams) -> Tuple[jnp.ndarray, jnp.ndarray]:
...
@jit
def make_decision_enhanced(state: EnhancedNetworkState, key: jax.random.PRNGKey) -> Tuple[int, EnhancedNetworkState]:
...
from phase04_research import (
    OptimizedGridWorld, compute_gradient_vectorized, 
    step_vectorized, StreamingDataLogger,
    update_dopamine_with_rpe, compute_dopamine_modulation
)
...
def main():
    ...
    print(f"  Features:")
    print(f"    ✓ Connection-type specific initialization")
    print(f"    ✓ Homeostatic plasticity") 
    print(f"    ✓ Metaplasticity and weight-dependent learning")
    print(f"    ✓ Population coding for inputs")
    print(f"    ✓ Separate E/I synaptic dynamics\n")
    ...
    for i in range(args.num_episodes):
        ...
        print(f"  ✅ Rewards: {result['metadata']['rewards_collected']}, "
