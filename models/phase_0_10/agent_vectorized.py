...
def create_connectivity_vectorized(key: random.PRNGKey, params,
    ...
    if len(sensory_indices) > 0:
        ...
        w_mask = w_mask.at[sensory_indices[:, None],
                           jnp.arange(n_in)].set(input_mask)
    ...
    if len(sens_indices) > 0 and len(proc_indices) > 0:
        ...
        sens_proc_probs = random.uniform(
            keys[1], (len(proc_indices), len(sens_indices)))
        sens_proc_mask = sens_proc_probs < params.P_SENSORY_PROCESSING

        # Use meshgrid to create index arrays
        proc_idx_grid, sens_idx_grid = jnp.meshgrid(
            proc_indices, sens_indices, indexing='ij')
        ...
        w_mask = w_mask.at[proc_flat[mask_flat],
                           n_in + sens_flat[mask_flat]].set(True)
    ...
    if len(proc_indices) > 0:
        ...
        dist_matrix = jnp.minimum(
            dist_matrix, n_proc - dist_matrix)  # Ring topology
        ...
        prob_matrix = jnp.where(
            ee_mask & is_local, params.P_PROC_PROC_LOCAL, prob_matrix)
        prob_matrix = jnp.where(ee_mask & ~is_local,
                                params.P_PROC_PROC_DIST, prob_matrix)
    ...
    if len(readout_indices) > 0 and len(exc_proc_indices) > 0:
        proc_read_probs = random.uniform(
            keys[3], (len(readout_indices), len(exc_proc_indices)))
        ...
        w_mask = w_mask.at[read_flat[mask_flat],
                           n_in + proc_flat[mask_flat]].set(True)
        if params.LEARN_PROCESSING_READOUT:
            w_plastic = w_plastic.at[read_flat[mask_flat],
                                     n_in + proc_flat[mask_flat]].set(True)
