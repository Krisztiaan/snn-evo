        ...
        for i_idx, i in enumerate(proc_indices):
            for j_idx, j in enumerate(proc_indices):
                ...
                if proc_e_mask[i_idx] and proc_e_mask[j_idx]:  # E→E
                    p = params.P_PROC_PROC_LOCAL if is_local[i_idx, j_idx] else params.P_PROC_PROC_DIST
...
class SnnAgent:
    ...
    def run_episode(self, episode_key: random.PRNGKey, episode_num: int, exporter: DataExporter, 
        ...
        for step in range(max_steps):
            ...
            # Debug: Log input activity occasionally
            if step % 1000 == 0 and self.config.exp_config.monitor_rates:
                max_input = float(jnp.max(input_channels))
                mean_input = float(jnp.mean(input_channels))
                if max_input > 0:
                    print(f"\n  [Step {step}] Gradient: {obs.gradient:.3f}, Input activity: mean={mean_input:.3f}, max={max_input:.3f}")
