...
def _get_adaptive_learning_rate(state: AgentState, params: NetworkParams) -> float:
    ...
    adaptive_rate = params.BASE_LEARNING_RATE * \
        (decay_rate ** state.episodes_completed)
...
def _three_factor_learning_optimized(
    ...
    total_reward_signal = actual_reward_component + \
        gradient_reward_component + td_error * 0.3
    ...
    def run_episode(self, episode_key: random.PRNGKey, episode_num: int, exporter: Optional[DataExporter] = None,
        ...
        for step in range(max_steps):
            ...
            if progress_callback and (step % 500 == 0 or step == max_steps - 1):
                ...
                progress_callback(step, max_steps, rewards_collected,
                                  elapsed, actual_rewards_collected)
        ...
        for i in range(self.config.exp_config.n_episodes):
            ...
            if i > 0:
                ...
                if i >= 5:
                    ...
                    recent_actual = sum(s['actual_rewards_collected']
                                        for s in all_summaries[-5:]) / 5
                    early_actual = sum(s['actual_rewards_collected']
                                       for s in all_summaries[:5]) / 5
