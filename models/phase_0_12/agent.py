...
def _get_adaptive_learning_rate(state: AgentState, params: NetworkParams) -> float:
    ...
    boost_factor = jnp.where(
        state.reward_boost_timer > 0,
        params.REWARD_BOOST_FACTOR,
        1.0
    )
...
def _reset_episode_state(state: AgentState, params: NetworkParams, soft_reset: bool = True) -> AgentState:
    ...
    new_temperature = jnp.maximum(
        state.current_temperature * params.TEMPERATURE_DECAY,
        params.FINAL_ACTION_TEMPERATURE
    )
    ...
    def run_episode(self, episode_key: random.PRNGKey, episode_num: int, exporter: Optional[DataExporter] = None, 
        ...
        for step in range(max_steps):
            ...
            if progress_callback and (step % 500 == 0 or step == max_steps - 1):
                ...
                progress_callback(step, max_steps, rewards_collected, elapsed, actual_rewards_collected)
        ...
        summary = {
            ...
            "rewards_collected": actual_rewards_collected,  # Fixed: use manually tracked count
        ...
        for i in range(self.config.exp_config.n_episodes):
            ...
            if i > 0:
                ...
                if i >= 5:
                    ...
                    recent_actual = sum(s['actual_rewards_collected'] for s in all_summaries[-5:]) / 5
                    early_actual = sum(s['actual_rewards_collected'] for s in all_summaries[:5]) / 5
                    actual_improvement = recent_actual - early_actual
                    print(f"  Actual reward improvement (last 5 vs first 5): {actual_improvement:+.2f}")
