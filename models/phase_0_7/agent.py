...
def _neuron_step(
    ...
    syn_current_e_new = state.syn_current_e * jnp.exp(-1.0 / params.TAU_SYN_E) + jnp.maximum(
        0, syn_input
    )
    syn_current_i_new = state.syn_current_i * jnp.exp(-1.0 / params.TAU_SYN_I) + jnp.minimum(
        0, syn_input
    )
...
@partial(jit, static_argnames=["params"])
def _learning_step(
    state: AgentState, reward: float, prev_value: float, params: NetworkParams
) -> AgentState:
    ...
    dopamine_response = (
        state.dopamine * decay_d + params.BASELINE_DOPAMINE * (1 - decay_d) + td_error * 0.5
    )
    ...
    dw_rec = (
        params.BASE_LEARNING_RATE * dopamine_modulation * new_eligibility_rec
        - params.WEIGHT_DECAY * w_rec
    )
...
class SnnAgent:
    ...
    def _initialize_state(self, key: random.PRNGKey) -> AgentState:
        ...
        w = w.at[main_row_slice, in_col_slice].set(
            jnp.where(
                in_main_conn,
                random.normal(subkey2, (num_main, num_input)) * p.W_STD_SCALE + p.W_IN_MAIN_MEAN,
                0,
            )
        )
        ...
        w = w.at[main_row_slice, clock_col_slice].set(
            jnp.where(
                clock_main_conn,
                random.normal(subkey2, (num_main, num_clock)) * p.W_STD_SCALE + p.W_CLOCK_MAIN_MEAN,
                0,
            )
        )
        ...
        w_main = jnp.where(
            main_main_conn,
            random.normal(subkey2, (num_main, num_main)) * p.W_STD_SCALE + p.W_MAIN_MAIN_MEAN,
            0,
        )
        ...
        w_motor_in = jnp.where(
            main_motor_conn,
            random.normal(subkey2, (num_motor, num_main)) * p.W_STD_SCALE + p.W_MAIN_MOTOR_MEAN,
            0,
        )
    ...
    def run_episode(
        ...
        motor_slice = slice(
            self.params.NUM_MAIN_NEURONS,
            self.params.NUM_MAIN_NEURONS + self.params.NUM_MOTOR_NEURONS,
        )
        ...
        for step in range(self.config.world_config.max_timesteps):
            ...
            self.state = _neuron_step(
                self.state, input_spikes, clock_spikes, self.params, neuron_key
            )
            ...
            if (
                self.state.steps_since_decision > 0
                and self.state.steps_since_decision % self.params.DECISION_WINDOW == 0
            ):
            ...
            self.state = self.state._replace(
                steps_since_decision=self.state.steps_since_decision + 1
            )
            ...
            exporter.log(
                ...
                behavior={
                    "action": int(self.state.current_action),
    ...
    def run_experiment(self):
        ...
        with DataExporter(
            ...
            for i in range(self.config.exp_config.n_episodes):
                ...
                print(
                    f"  Episode Summary: Total Reward: {summary['total_reward']:.2f}, Rewards Collected: {summary['rewards_collected']}, Steps: {summary['steps_taken']}"
                )
