...
const state = {
    ...
    agent: null, trail: null, rewards: [], grid: null,
    experiments: {}, currentData: null, currentStep: 0,
    isPlaying: false, playInterval: null,
    charts: {}, activeChart: 'behavior',
};
...
function attachEventListeners() {
    ...
    DOM.timeline.addEventListener('input', (e) => setStep(parseInt(e.target.value)));
    DOM.analyticsPanel.querySelector('.analytics-tabs').addEventListener('click', (e) => {
...
window.loadData = async function() {
    ...
    try {
        const response = await fetch(`/api/episode-data?path=${encodeURIComponent(path)}&episode=${episodeId}`);
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        const data = await response.json();
...
function populateModelSelect() {
    ...
    Object.keys(state.experiments).sort().forEach(modelName => {
...
function createTrail() {
    ...
    const material = new THREE.LineBasicMaterial({ color: CONFIG.TRAIL_COLOR, linewidth: 2 });
...
function setStep(step) {
    ...
    state.currentStep = Math.max(0, Math.min(step, state.currentData.timeline.length - 1));
...
function updateScene() {
    ...
    const trailPositions = timeline.slice(0, state.currentStep + 1).map(s => new THREE.Vector3(s.position[0] + 0.5, 0.1, s.position[1] + 0.5));
    ...
    state.rewards.forEach((r, i) => {
        ...
        r.mesh.material.color.set(r.collected ? CONFIG.REWARD_COLLECTED_COLOR : CONFIG.REWARD_COLOR);
...
function updateUI() {
    ...
    DOM.stepDisplay.textContent = `Step: ${stepData.step} / ${state.currentData.metadata.totalSteps - 1}`;
    DOM.info.step.textContent = stepData.step;
    DOM.info.pos.textContent = `${stepData.position[0].toFixed(1)}, ${stepData.position[1].toFixed(1)}`;
    DOM.info.action.textContent = ['Up', 'Right', 'Down', 'Left'][stepData.action] || 'N/A';
    ...
    DOM.info.potential.textContent = neural ? neural.mean_potential.toFixed(2) : 'N/A';
    DOM.info.firingRate.textContent = neural ? neural.firing_rate_hz.toFixed(2) : 'N/A';
...
function toggleAnalytics() {
    ...
    DOM.toggleAnalyticsBtn.textContent = isHidden ? '📊 Hide Analytics' : '📊 Show Analytics';
    if (isHidden) {
        Object.values(state.charts).forEach(chart => chart.resize());
    }
}
...
function prepareChartData() {
    ...
    const chartOptions = (title, legend, yAxis, series) => ({
        ...
        xAxis: { type: 'category', data: steps, axisLine: { lineStyle: { color: CONFIG.CHART_TEXT } } },
        yAxis: yAxis,
        series: series,
    ...
    state.charts.behavior.setOption(chartOptions(
        'Behavioral Metrics', ['Cumulative Reward', 'Gradient'],
        [
            { type: 'value', name: 'Reward', axisLine: { lineStyle: { color: CONFIG.CHART_TEXT } } },
            { type: 'value', name: 'Gradient', axisLine: { lineStyle: { color: CONFIG.CHART_TEXT } } }
        ],
        [
            { name: 'Cumulative Reward', type: 'line', data: cumulativeRewards, itemStyle: { color: CONFIG.CHART_PRIMARY } },
            { name: 'Gradient', type: 'line', yAxisIndex: 1, data: timeline.map(s => s.gradient), itemStyle: { color: CONFIG.CHART_SECONDARY } }
        ]
    ), true);

    state.charts.neural.setOption(chartOptions(
        'Neural Dynamics', ['Firing Rate (Hz)', 'Mean Potential (mV)'],
        [
            { type: 'value', name: 'Hz', axisLine: { lineStyle: { color: CONFIG.CHART_TEXT } } },
            { type: 'value', name: 'mV', axisLine: { lineStyle: { color: CONFIG.CHART_TEXT } } }
        ],
        [
            { name: 'Firing Rate (Hz)', type: 'line', data: timeline.map(s => s.neural?.firing_rate_hz), itemStyle: { color: CONFIG.CHART_NEURAL_1 } },
            { name: 'Mean Potential (mV)', type: 'line', yAxisIndex: 1, data: timeline.map(s => s.neural?.mean_potential), itemStyle: { color: CONFIG.CHART_NEURAL_2 } }
        ]
    ), true);

    state.charts.learning.setOption(chartOptions(
        'Learning Signals', ['Dopamine', 'Value Estimate'],
        { type: 'value', axisLine: { lineStyle: { color: CONFIG.CHART_TEXT } } },
        [
            { name: 'Dopamine', type: 'line', data: timeline.map(s => s.neural?.dopamine), itemStyle: { color: CONFIG.CHART_LEARNING_1 } },
            { name: 'Value Estimate', type: 'line', data: timeline.map(s => s.neural?.value_estimate), itemStyle: { color: CONFIG.CHART_LEARNING_2 } }
        ]
    ), true);
}
