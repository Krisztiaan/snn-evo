        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with DataExporter(
                ...
                with exporter.start_episode(0) as episode:
                    ...
                    episode.log_static_data("test_static", {
            ...
            for compression, level in compressions:
                ...
                with DataExporter(**config) as exporter:
                    with exporter.start_episode(0) as episode:
                        episode.log_timestep(
                            timestep=0,
                            neural_state={"test": np.ones(1000)}
                        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with DataExporter(
                ...
                with exporter.start_episode(0) as episode:
                    ...
                    # Test static data
                    episode.log_static_data("metadata", {
...
class TestPerformanceFeatures:
    ...
    def test_profiling(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with DataExporter(
                experiment_name="test_profiling",
                output_base_dir=temp_dir,
...
class TestConcurrency:
    ...
    def test_concurrent_episodes(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with DataExporter(
                experiment_name="test_concurrent",
                output_base_dir=temp_dir
            ) as exporter:
                def write_episode(episode_id):
                    with exporter.start_episode(episode_id) as episode:
                        for t in range(100):
                            episode.log_timestep(
                                timestep=t,
                                neural_state={"thread_id": episode_id, "data": np.random.randn(100)}
                            )
            ...
            with DataExporter(
                ...
                for ep in range(3):
                    with exporter.start_episode(ep) as episode:
                        ...
                        # Episode summary
                        episode.log_static_data("summary", {
            ...
            with DataExporter(
                ...
                with exporter.start_episode(0) as episode:
                    ...
                    for t in range(1000):
                        ...
                        # Stimulus events
                        if t % 200 == 0:
                            episode.log_event("stimulus", t, {
                                "type": "grating",
                                "orientation": t // 200 * 45
                            })
        ...
        with DataExporter(
            ...
            with exporter.start_episode(0) as episode:
                ...
                for t in range(1000):
                    ...
                    episode.log_timestep(
                        timestep=t,
                        neural_state={"v": membrane_potential},
