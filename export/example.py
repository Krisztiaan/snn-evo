...
def main():
    ...
    with DataExporter(
        ...
        neurons = {
            ...
            "neuron_types": np.random.choice([0, 1], n_neurons),
        ...
        initial_weights[np.random.rand(
            n_neurons, n_neurons) > 0.1] = 0  # 90% sparse
        ...
        for episode_id in range(n_episodes):
            ...
            with exporter.start_episode(episode_id) as episode:
                ...
                for t in range(n_timesteps):
                    ...
                    membrane_potential = np.random.randn(
                        n_neurons) * 20 - 70  # mV
    ...
    with ExperimentLoader(exp_dir) as loader:
        ...
        print(
            f"     - Neural states: {neural_states['timesteps'].shape[0]} samples")
        print(
            f"       Mean membrane potential: {neural_states['membrane_potential'].mean():.2f} mV")
        ...
        if rewards:
            print(
                f"     - Rewards: {len(rewards['timesteps'])} non-zero rewards")
        ...
        if weight_changes and 'timesteps' in weight_changes:
            print(
                f"     - Weight changes: {len(weight_changes['timesteps'])} changes")
