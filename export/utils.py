...
def optimize_jax_conversion(data: Dict[str, Any]) -> Dict[str, np.ndarray]:
    ...
    """
    if not jax_available:
        return {k: ensure_numpy(v) if hasattr(v, "__array__") else v for k, v in data.items()}
...
def load_episode_data(episode_dir: Union[str, Path]) -> dict:
    ...
    if neural_files:
        ...
        data["neural_states"] = {key: np.concatenate(arrays) for key, arrays in all_neural.items()}
...
def load_experiment_summary(experiment_dir: Union[str, Path]) -> dict:
    ...
    return {"metadata": metadata, "episode_summaries": summaries}
...
def align_timesteps(
    sparse_data: dict, reference_timesteps: np.ndarray, default_value: Any = None
) -> np.ndarray:
...
def interpolate_sampled_data(
    sampled_data: dict, reference_timesteps: np.ndarray, method: str = "nearest"
) -> dict:
...
def create_timestep_mapping(
    behavior_timesteps: np.ndarray, neural_timesteps: np.ndarray, spike_timesteps: np.ndarray = None
) -> dict:
    """Create mapping between different data stream timesteps.

    Returns dict with mappings for efficient lookup.
    """
    mapping = {"behavior_to_neural": {}, "neural_to_behavior": {}, "spike_bins": {}}
    ...
    for i, b_ts in enumerate(behavior_timesteps):
        ...
        if idx > 0 and (
            idx == len(neural_timesteps)
            or abs(neural_timesteps[idx - 1] - b_ts) < abs(neural_timesteps[idx] - b_ts)
        ):
            idx -= 1
