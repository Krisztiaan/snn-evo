        ...
        if not isinstance(neural_state, dict):
            ...
            for key, value in neural_state.items():
                if hasattr(value, "shape"):
                    ...
                    if value.ndim > 2:
                        warnings.append(
                            f"neural_state[{key}] has {value.ndim} dimensions, expected 1 or 2"
                        )
...
def validate_weight_change(
    timestep: int, synapse_id: Union[int, tuple], old_weight: float, new_weight: float, **kwargs
) -> List[str]:
    ...
    if "neuron_ids" not in neurons:
        ...
        for key, value in neurons.items():
            if hasattr(value, "__len__") and len(value) != n_neurons:
                warnings.append(
                    f"neurons[{key}] length {len(value)} doesn't match n_neurons {n_neurons}"
                )
        ...
        for key, value in connections.items():
            if key not in required_connection_fields and hasattr(value, "__len__"):
                if len(value) != n_connections:
                    warnings.append(
                        f"connections[{key}] length {len(value)} doesn't match n_connections {n_connections}"
                    )
