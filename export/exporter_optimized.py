...
class BufferedDataset:
    ...
    def __init__(
        ...
        with _HDF5_LOCK:
            ...
            create_kwargs = {"name": name, "shape": shape, "maxshape": maxshape, "dtype": dtype}
        ...
        with _HDF5_LOCK:
            if new_size > self.allocated_size:
                ...
                self.allocated_size = int(
                    max(new_size * self.growth_factor, self.allocated_size + self.chunk_size)
                )
            ...
            if new_size > self.allocated_size:
                with self._lock:
                    ...
                    if new_size > self.allocated_size:
                        self.allocated_size = int(
                            max(
                                new_size * self.growth_factor, self.allocated_size + self.chunk_size
                            )
                        )
                        self.dataset.resize(self.allocated_size, axis=0)
    ...
    def __del__(self):
        ...
        if (
            hasattr(self, "memory_tracker")
            and self.memory_tracker
            and hasattr(self, "buffer_memory")
        ):
...
class OptimizedEpisode:
    ...
    def _append_neural_state(self, timestep: int, data: Dict[str, Any]):
        ...
        if self.realtime_stats:
            data_size = sum(v.nbytes if hasattr(v, "nbytes") else 0 for v in data.values())
    ...
    def log_event(self, event_type: str, timestep: int, data: Dict[str, Any]):
        ...
        total_events = sum(
            len(values)
            for event_data in self.event_buffers.values()
        ...
        with _HDF5_LOCK:
            ...
            def save_array(parent_group, key: str, numpy_value: np.ndarray):
                ...
                if numpy_value.ndim == 0 or numpy_value.size == 1:
                    ...
                    if compression == "lzf":
                        parent_group.create_dataset(key, data=numpy_value, compression=compression)
                    else:
    ...
    def _write_weight_changes(self):
        ...
        with _HDF5_LOCK:
            ...
            elif "source_id" in all_changes[0]:
...
class OptimizedDataExporter:
    ...
    def __init__(
        ...
        if not self.no_write and compression == "gzip":
    ...
    def save_runtime_info(self):
        ...
        runtime_info = {
            ...
            "hdf5_driver": self.h5_file.driver if not self.no_write else "none",
            "hdf5_libver": str(self.h5_file.libver) if not self.no_write else "none",
        }
    ...
    def log(self, **kwargs):
        ...
        if "synapse_id" in kwargs and "old_weight" in kwargs and "new_weight" in kwargs:
            # Filter kwargs for the method
            allowed_keys = ["timestep", "synapse_id", "old_weight", "new_weight"]
            method_kwargs = {k: v for k, v in kwargs.items() if k in allowed_keys}
            # Pass through any extra kwargs
            extra_kwargs = {k: v for k, v in kwargs.items() if k not in allowed_keys}
            self.current_episode.log_weight_change(**method_kwargs, **extra_kwargs)
        elif "event_type" in kwargs and "data" in kwargs:
            self.current_episode.log_event(
                event_type=kwargs["event_type"], timestep=timestep, data=kwargs["data"]
            )
        else:
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        if self.adaptive_compressor:
            comp_stats = {}
            for name, data in self.adaptive_compressor.stats.items():
                if data["count"] > 0:
                    comp_stats[name] = {"avg_ratio": data["ratio"], "avg_time": data["time"]}
